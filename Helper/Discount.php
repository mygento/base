...
class Discount
{
     ...
     * @param \Magento\Sales\Model\Order\Invoice|\Magento\Sales\Model\Order\Creditmemo|\Magento\Sales\Model\Order $entity
    ...
    public function getRecalculated(
        ...
        $this->generalHelper->addLog(
            "== START == Recalculation of entity prices. Helper Version: ".self::VERSION.".  Entity class: ".get_class(
                $entity
        ...
        if ($globalDiscount !== 0.00) {
            ...
            $this->generalHelper->addLog(
                "SplitItems and DoCalculation set to true because of global Discount (e.g. reward points)"
            );
        }
        switch (true) {
    ...
    protected function _buildItem($item, $price, $taxValue = '')
    {
        ...
        $this->generalHelper->addLog(
            "Item id: {$item->getId()}. Orig price: {$price} Item rowTotalInclTax: {$item->getData('row_total_incl_tax')} PriceInclTax of 1 piece: {$price}. Result of calc:"
        );
    ...
    public function getProcessedItem($item)
    {
        ...
        $price    = !is_null($item->getData(self::NAME_UNIT_PRICE)) ? $item->getData(
            self::NAME_UNIT_PRICE
